...
def trainer():
    ...
    train_dataset, valid_dataset = load_datasets(path_dataset=args.dataset_path, word2id_en=en_word2idx,
                                                 word2id_vi=vi_word2idx, buffer_size=args.buffer_size,
                                                 streaming=args.streaming)

    valid_dataloader = DataLoader(dataset=valid_dataset, batch_size=args.batch_size, collate_fn=translation_collate_fn)
    ...
    for epoch in range(epochs):
        random_indices = torch.randperm(total_samples)[:samples_per_epoch].tolist()

        epoch_dataset = Subset(train_dataset, random_indices)
        ...
        for datapoint in progress_bar:
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                ...
                display_loss_total = total_loss.item() * accumulation_steps
